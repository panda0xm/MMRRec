...
def scatter_add_pseudo(src, index, dim=0, dim_size=None):  
    # 初始化一个全零的张量，用于累加结果  
    if dim_size is None:  
        dim_size = src.max().item() + 1 if dim == 0 else src.size(dim)  
...
def get_sparse_laplacian(edge_index, edge_weight, num_nodes, normalization='none'):
    import torch_scatter1
    from torch_scatter1 import scatter_add
    row, col = edge_index[0], edge_index[1]
    deg = scatter_add(edge_weight, row, dim=0, dim_size=num_nodes)
    #deg = scatter_add_pseudo(edge_weight, row, dim=0, dim_size=num_nodes)
